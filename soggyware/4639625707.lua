-- basic i know they patched a lot of things and their anti cheat is server sided so no bypass :cringe:
...
local function aimbot()
    ...
    local function GetClosestPlayer()
            ...
            for _, v in next, Players:GetPlayers() do
                if v ~= LocalPlayer then
                    if v.Character and v.Character[Environment.Settings.LockPart] then
                        ...
                        if Environment.Settings.AliveCheck and v.Character.Humanoid.Health <= 0 then
                        end
                        if
                            Environment.Settings.WallCheck and
                        ...
                        local Distance =
                            ...
                            Vector2.new(Vector.X, Vector.Y)).Magnitude
        ...
        elseif
            (Vector2.new(UserInputService:GetMouseLocation().X, UserInputService:GetMouseLocation().Y) -
                Vector2.new(
                    Camera:WorldToViewportPoint(Environment.Locked.Character[Environment.Settings.LockPart].Position).X,
                    Camera:WorldToViewportPoint(Environment.Locked.Character[Environment.Settings.LockPart].Position).Y
        ...
        ServiceConnections.RenderSteppedConnection =
            ...
            function()
                if Environment.FOVSettings.Enabled and Environment.Settings.Enabled then
                    ...
                    Environment.FOVCircle.Position =
                        Vector2.new(UserInputService:GetMouseLocation().X, UserInputService:GetMouseLocation().Y)
                else
    ...
    if Environment.Settings.ReloadOnTeleport then
        ...
        if queueonteleport then
            queueonteleport(
                game:HttpGet("https://raw.githubusercontent.com/Exunys/Aimbot-V2/main/Resources/Scripts/Main.lua")
            )
        else
    ...
    SendNotification(
        Title,
        "Aimbot script successfully loaded! Check the GitHub page on how to configure the script.",
        5
    )
end
